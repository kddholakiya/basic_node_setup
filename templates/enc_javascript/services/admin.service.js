...
const { getEpoch, encryptionData, generateRandomPassword,  encryptVerifyToken,} =
...
async addUser(req, callback) {
  try {
    const currentTime = await getEpoch();

    const adminUser = await user.findOne({ _id: req.user._id, accountType: { $in: [1, 2] } });
    ...
    // 🔹 Save user
    const userCreate = new user(newUser);
