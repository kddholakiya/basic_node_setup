...
import { createDat, decrypt } from "./Crypto.js";
...
export default class CommanController {
  ...
  async DecryptPayload(req, res, next) {
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        const envKey = process.env.chadb_url + process.env.hard_cord_url
        const encIv = process.env.snap_art + process.env.sudo_apply
        const KEY = envKey + "62";
        const IV = encIv + "vn";
  ...
  async encryptionData(type, data) {
