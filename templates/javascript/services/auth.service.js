...
const { getEpoch, CreateJwt,  encryptVerifyToken, generateRandomPassword,  } = new CommanController();
var connectHost;
...
export default class authService {
    ...
    async login(data, callback) {
        try {
            ...
            const userData = await user.findOne({ $or: [{ email: smallCaseEmamil }, { userName: email }], });
            if (userData) {
                ...
                if ((userData.accountType == 1 || userData.accountType == 2 || userData.accountType == 3) && data.type == 2) {
                    ...
                    if (userData?.tempPassword) {
                        ...
                        const tempHashedBuffer = scryptSync(data?.password, tempSalt, 64);
                        const tempKeyBuffer = Buffer.from(tempKey, 'hex');
                    ...
                    if (match) {
                        ...
                        let userLoginData = await userLogin.findOne(loginQuery);
                        if (userLoginData) {
                        ...
                        var resData = { token: jwt, userid : userData._id ,  accountType: userData.accountType, }
                        var userUpdate = await user.updateOne({ _id: userData?._id }, { authToken: jwt }, { new: true });
        ...
        try {
            ...
            if (userData) {
                ...
                if (!match) {
                    ...
                    var userUpdate = await user.updateOne({ _id: userData?._id }, { password: newHashedPassword }, { new: true })
                    if (userUpdate['acknowledged']) {
    ...
    async editProfile(req, callback) {
        try {
            ...
            if (userData) {
                ...
                var userUpdate   = await user.updateOne({ _id: userData?._id }, updatedData, { new: true })
                if (userUpdate['acknowledged']) {
    ...
    async logout(req, callback) {
        try {
            const deviceType = req?.body?.deviceType;
            var userLoginData = await userLogin.findOne({ userId: req.user._id, deviceType: deviceType, authToken: req['userToken'] })
    ...
    async forgetPassword(req, callback) {
        try {
            ...
            if (userData?.isEmailVerified == false || userData?.isAdminVerified == false || userData?.isEmailReverified == false) {
                ...
                const userUpdate = await user.updateOne({ _id: userData?._id }, updateData, { new: true });
        ...
        try {
            ...
            if (userData) {
                ...
                if (Number(currentTime) <= Number(userData.emailTokenExpiredAt)) {
                    ...
                    var userUpdate = await user.updateOne({ _id: userData._id }, updateData, { new: true })
                    if (userUpdate['acknowledged']) {
    ...
    async refreshToken(req, callback) {
        try {
            ...
            jwt.verify(trimToken, decryptApisecretkey, async (err, decoded) => {
                ...
                if (err) {
                    ...
                    return callback(null, { 'status': HttpCodes['API_SUCCESS'], 'msg': 'notValid', 'code': HttpCodes['OK'], 'data': refreshToken });
                } else {
                    return callback(null, { 'status': HttpCodes['API_FAILURE'], 'msg': 'ValidToken', 'code': HttpCodes['NOT_MODIFIED'], 'data': refreshToken });
                }
            });
        } catch (error) {
            return callback(null, { 'status': HttpCodes['API_FAILURE'], 'msg': 'SomethingWrong', 'code': HttpCodes['BAD_REQUEST'], 'data': {} });
