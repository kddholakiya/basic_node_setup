...
export default class CommanController {
  ...
  async CreateJwt(data: any) {
    ...
    const decryptJWT_EXPIRATION: any = process.env.JWT_EXPIRATION;
  ...
  async VerifyJwt(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const decryptWebKey = process.env.WebKey;  // adbohkjfbdshjbsdjffkbj use this value for swagger 
      const decryptMobileKey = process.env.mobileKey;
