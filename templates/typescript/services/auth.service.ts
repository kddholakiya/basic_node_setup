        ...
        try {
            ...
            jwt.verify(trimToken, decryptApisecretkey, async (err, decoded) => {
                ...
                if (err) {
                    ...
                    return callback(null, { 'status': HttpCodes['API_SUCCESS'], 'msg': 'notValid', 'code': HttpCodes['OK'], 'data': updaeUser });
                } else {
                    return callback(null, { 'status': HttpCodes['API_FAILURE'], 'msg': 'ValidToken', 'code': HttpCodes['NOT_MODIFIED'], 'data': updaeUser });
                }
            });
        } catch (error) {
            return callback(null, { 'status': HttpCodes['API_FAILURE'], 'msg': 'SomethingWrong', 'code': HttpCodes['BAD_REQUEST'], 'data': {} });
